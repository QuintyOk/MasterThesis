"""Estimate the SCE final-choice multinomial logit model.

The final choice incorporates each respondent's parking-rights decision. The
script estimates the corresponding Biogeme model, reports fitted probabilities,
and runs the same individual and combined policy simulations as the base-choice
analysis.
...
# Prepare the final choice and availability variables for Biogeme.
...
data["final_choice_num"] = data["final_choice"].map(choice_map).astype(int)
...
CHOICE = Variable("final_choice_num")

print("Choice variable defined (final_choice)...")
...
biogeme.model_name = "Final Choice MNL Model"
...
# Calculate fitted final-choice probabilities.
simulate = {
...
print("\n=== Average probabilities per experiment (final choice)")
...
def simulate_scenario(choice_data, V, AV, results, scenario, label):
    """Simulate average final-choice probabilities for one scenario."""
    ...
    for k, v in scenario.items():
        if callable(v):
            print(f"  - {k}: priority_rule(distance > 20km)")
        else:
            print(f"  - {k}: {v}")
    ...
    for col, val in scenario.items():
        scenario_data[col] = val(scenario_data) if callable(val) else val
...
# Calculate modal changes in percentage points.
def compute_pp_change(df, scenario_col):
    """Add percentage-point changes from the base case."""
    df_pct = df.copy()
    ...
    for mode in ["Prob_car", "Prob_pr", "Prob_vp", "Prob_pt", "Prob_bike"]:
        ...
        if base_val != 0:
            df_pct[f"{mode}_pp_change"] = (df_pct[mode] - base_val) * 100
        else:
            df_pct[f"{mode}_pp_change"] = None

    return df_pct

parking_cost_pct = compute_pp_change(parking_scenarios_df, "parking_cost")
parking_guaranteed_pct = compute_pp_change(parking_guaranteed_df, "parking_guaranteed")
pr_reward_pct = compute_pp_change(pr_reward_df, "pr_reward")

print("\n--- Parking Cost (pp change) ---")
print(parking_cost_pct)

print("\n--- Parking Guaranteed (pp change) ---")
print(parking_guaranteed_pct)

print("\n--- P+R Reward (pp change) ---")
print(pr_reward_pct)
